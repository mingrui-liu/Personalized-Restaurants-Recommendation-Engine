...
public class GeoRecommendation {
	...
	public List<Item> recommendItems(String userId, double lat,double lon){
		...
		// step 1 : get all favoriate itemids
		MySQLConnection connection = new MySQLConnection();
		Set<String> favoriatedItemIds = connection.getFavoriteItemIds(userId);
		// Step 2, get all categories, sort by count
		// {"sports": 5, "music": 3, "art": 2}
		Map<String, Integer> allCategories = new HashMap<>();
		for(String itemId : favoriatedItemIds) {
		...
		for (Entry<String, Integer> category : categoryList) {
			...
			for (Item item : items) {
				if (!favoriatedItemIds.contains(item.getItemId()) && !visitedItemIds.contains(item.getItemId())) {
